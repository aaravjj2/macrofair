...
The product ships with a typed FastAPI backend and a Next.js frontend designed for judge-speed comprehension. The homepage now leads with a featured dislocation and a flagship finding card before the ranked table. In the deterministic demo snapshot, the CPI June 2026 contract contributes 69.82% of total absolute dislocation mass and is 3.146x larger than the second-ranked gap. This gives a memorable headline and a concrete path into the detail view.
