...
def test_zerve_submission_package_has_expected_shape() -> None:
    ...
    assert package["findings"]["third"]["dominant_drift_category"] == "inflation"
    assert package["ranked_snapshot_summary"]["count"] >= 5
    assert isinstance(package["findings_report_hash"], str)
    assert len(package["findings_report_hash"]) == 64
